...
class HighLevelEncoder(nn.Module):
    def __init__(self):
        ...
        self.reduction = AdaptiveMaxAbsolutePooling(4096)

        cnn_layers = []
        prev_filter = 1
        for filter_size in [8, 16, 16, 32, 32, 64, 64, 128, 128]:
            cnn_layers.extend(
                [
                    nn.Conv1d(prev_filter, filter_size, kernel_size=3),
                    nn.ReLU(),
                    MaxAbsolutePooling(pool_size=3, stride=2)
                ]
            )
